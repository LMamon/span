    ...
    void Renderer::render(const Grid& grid,
        ...
        for (std::size_t y = 0; y < grid.height(); ++y) {
            ...
            for (std::size_t x = 0; x < grid.width(); ++x) {
                ...
                // Most recent team to visit this cell.
                auto visited = visited_by.find(position);

                if (visited != visited_by.end()) {
                    symbol = "■";
                    symbol_color = team_color(visited->second);
                }

                // Planned paths and starting positions.
                for (const Team& team : teams) {
                    for (const Agent& agent : team.agents) {
                        ...
                        if (position == agent.start()) is_start = true;
                    }
                }

                // Current agent occupancy always wins over visited state.
                for (const Team& team : teams) {
                ...
                const bool is_goal = std::find(goals.begin(), goals.end(), position) != goals.end();
